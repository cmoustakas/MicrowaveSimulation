...
static inline void updatePositionOnSphere(glm::vec3 &position,
  ...
  position.x = sphere.m_ray * sin(sphere.m_theta) * cos(sphere.m_fi);
  position.y = sphere.m_ray * sin(sphere.m_fi);
  position.z = sphere.m_ray * cos(sphere.m_theta) * cos(sphere.m_fi);
}
...
void CameraHandler::updateCamera(float theta, float fi,
  ...
  constexpr float low = -glm::pi<float>() / 2;
  constexpr float high = glm::pi<float>() / 2;
  m_sphere.m_fi += fi;
